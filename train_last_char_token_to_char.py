...
wandb_project = 'owt'
wandb_run_name = 'gpt2_last_char_token_to_char' + str(time.time())
# data
...
def get_batch(split):
    ...
    # if len(enc.decode(x[0][x[0] != 65535].tolist())) > 1000:
    #     print(enc.decode(x[0][x[0] != 65535].tolist()), torch.sum(x[0] != 65535), len(enc.decode(x[0][x[0] != 65535].tolist())))
    #     input()

    x = torch.nn.utils.rnn.pad_sequence(x, batch_first=True, padding_value=65535)
