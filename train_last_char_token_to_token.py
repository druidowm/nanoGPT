...
from model_token_predicter import GPT_token_predictor
...
wandb_project = 'owt'
wandb_run_name = 'gpt2_last_char_token_to_token' + str(time.time())
# data
dataset = 'openwebtext_last_char_token_to_token'
...
val_data_out = np.memmap(os.path.join(data_dir, 'val_token_out.bin'), dtype=np.int32, mode='r')
...
def get_batch(split):
    ...
    new_batch_sizes = [get_batch_size(data_out[i:i+10*block_size]) for i in ix]


    # Right pad the batch to the maximum batch size
    
    x = [torch.from_numpy((data[i:i+batch_size]).astype(np.int64)) for i, batch_size in zip(ix, new_batch_sizes)]
    y = [torch.from_numpy((data_out[i:i+batch_size]).astype(np.int64)) for i, batch_size in zip(ix, new_batch_sizes)]
...
if init_from == 'scratch':
    ...
    model = GPT_token_predictor(gptconf, 50304)
